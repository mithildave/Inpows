...
def common_knowledge_checking(gg, token, common_knowledge_tags):
    ...
    if target.ner in common_knowledge_tags:
        api = mapping_ner_by_literal(gg, target.ner, common_knowledge_tags)

        if (target.ner == 'string'):
            api = ['string']
        elif (target.ner == 'condition'):
            api = ['conditions']
        elif (target.ner == 'hpc_concept'):
             api = ['hpc_concept']
...
def mapping_ner_by_literal(gg, ner, common_knowledge_tags):
    log.test('mapping ner:', ner)
    result = []
    for k in gg.api_dict.keys():
        log.test(common_knowledge_tags[ner], gg.api_dict[k].name)
        if common_knowledge_tags[ner] == gg.api_dict[k].name:
            result.append(k)
    return result
...
def grammar_check(tree_list, gg):
    log.log('checking grammar on cgts...')
    ...
    log.test('incomplete_node:', incomplete_node_list)
